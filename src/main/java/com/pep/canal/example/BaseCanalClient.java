...
import com.pep.business.ConvertEntityConfig;
import com.pep.business.ConvertEntityInit;
import com.pep.business.KafkaProducerWrap;
import com.pep.business.KafkaSinkProcessor;
import com.pep.canal.common.AESUtil;
...
public class BaseCanalClient {
    ...
    protected void doEtl(List<Entry> entrys) throws Exception {
        ...
        for (Entry entry : entrys) {
            ...
            if (entry.getEntryType() == EntryType.ROWDATA) {
                ...
                for (ConvertEntityConfig convertEntityConfig : convertEntityConfigList) {
                    ...
                    if (entry.getHeader().getSchemaName() != null && entry.getHeader().getTableName() != null && entry.getHeader().getSchemaName().equals(convertEntityConfig.getFrom_db()) && entry.getHeader().getTableName().equals(convertEntityConfig.getFrom_table())) {
                        ...
                        //为多少canal数据添加事务的执行时间，此字段将用于数仓数据的版本更细状态
                        for (RowData rowData : rowChage.getRowDatasList()) {
                            sourceMap.put(RowKeyStatic.ROW_TIMESTAMP, String.valueOf(System.currentTimeMillis()));
                            ...
                            sourceMap = convertMap(columnsList,sourceMap);
                            String json = ConvertEntityInit.convertEntity(sourceMap, convertEntityConfig);
                            logger.error(json);
                            String jsonEncode = AESUtil.encrypt(json, AESUtil.DEFAULT_PRIVATE_KEY);
                            KafkaSinkProcessor.toKafkaChannel.put(convertEntityConfig.getTo_topic() + KafkaProducerWrap.SEPARATOR + jsonEncode);
                        }
                    }
                }
            }
    ...
    private static Map<String, String> convertMap(List<CanalEntry.Column> columnsList,Map sourceMap) {
