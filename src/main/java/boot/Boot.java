...
public class Boot {
    ...
    public static void boot(Properties properties, Map<String, RecordListener> recordListeners) {
        ...
        //add kafkaSinkProcessor
        KafkaSinkProcessor kafkaSinkProcessor = new KafkaSinkProcessor();
        List<WorkThread> startStream = startWorker(etlRecordProcessor, recordGenerator,kafkaSinkProcessor);
    ...
    private static List<WorkThread> startWorker(EtlRecordProcessor etlRecordProcessor, RecordGenerator recordGenerator,KafkaSinkProcessor
            kafkaSinkProcessor) {
        List<WorkThread> ret = new LinkedList<>();
        ret.add(new WorkThread(etlRecordProcessor));
        ret.add(new WorkThread(recordGenerator));
        ret.add(new WorkThread(kafkaSinkProcessor));
